...
- **Per-core CPU load breakdown** — design doc already specs the format
...
## Shipped
- Real NOMINAL/CRITICAL status badge logic (with WARNING state)
- CPU / Memory / GPU live panels
- Elevation-aware graceful degradation (amber `--` fallback)
- Trend history charts (120s rolling, CPU/GPU/Memory)
- Elevation-on-demand (default non-elevated, in-app admin relaunch)
- HUD polish pass (glow, corner brackets, scan-lines, hover glow) — *in review*